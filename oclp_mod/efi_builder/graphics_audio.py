...
class BuildGraphicsAudio:
        ...
        if self.model in model_array.MacPro:
            if not self.constants.custom_model:
                for i, device in enumerate(self.computer.gpus):
                    logging.info(f"- 发现独立显卡 ({i + 1}): {utilities.friendly_hex(device.vendor_id)}:{utilities.friendly_hex(device.device_id)}")
                    ...
                    if device.pci_path and device.acpi_path:
                        logging.info(f"- 发现独立显卡 ({i + 1}) 在 {device.pci_path}")
                        if isinstance(device, device_probe.AMD):
                            logging.info("- 添加 Mac Pro, Xserve DRM 补丁")
            ...
            for i, device in enumerate(self.computer.gpus):
                if isinstance(device, device_probe.NVIDIA):
                    if (
                        ...
                        logging.info(f"- 启用 Web Driver 补丁给 GPU ({i + 1}): {utilities.friendly_hex(device.vendor_id)}:{utilities.friendly_hex(device.device_id)}")
                        if device.pci_path and device.acpi_path:
        ...
        if not self.constants.custom_model:
            for i, device in enumerate(self.computer.gpus):
                    ...
                    if device.pci_path != self.computer.dgpu.pci_path:
                        logging.info("- 设备路径和 GFX0 设备路径不同")
                        self.gfx0_path = device.pci_path
                        logging.info(f"- 设置 GFX0 设备路径: {self.gfx0_path}")
                        self.computer.dgpu.device_id = device.device_id
                        self.device_id = device.device_id
                        logging.info(f"- 发现 GPU 架构: {device.arch}")
                        ...
                        logging.info(f"- 发现 GFX0 设备路径: {self.gfx0_path}")
                        logging.info(f"- 发现 GPU 架构: {self.computer.dgpu.arch}")

        else:
            if not self.constants.custom_model:
                logging.info("- 无法找到 GFX0 设备路径，回退到已知逻辑")
    ...
    def _nvidia_mxm_patch(self, backlight_path) -> None:
        ...
        if self.model in ["iMac11,1", "iMac11,2", "iMac11,3", "iMac10,1"]:
            logging.info("- 添加 Nvidia 亮度控制和 DRM 补丁")
            ...
            if self.constants.custom_model and self.model == "iMac11,2":
                # iMac11,2 可以是 PciRoot(0x0)/Pci(0x3,0x0)/Pci(0x0,0x0) 或 PciRoot(0x0)/Pci(0x1,0x0)/Pci(0x0,0x0)
                # 当我们无法运行硬件检测时设置两个属性
    ...
    def _amd_mxm_patch(self, backlight_path) -> None:
       ...
       logging.info("添加 AMD DRM 补丁")
       if not support.BuildSupport(self.model, self.constants, self.config).get_kext_by_bundle_path("WhateverGreen.kext")["Enabled"] is True:
           # Ensure WEG is enabled as we need if for Backlight patching
           support.BuildSupport(self.model, self.constants, self.config).enable_kext("WhateverGreen.kext", self.constants.whatevergreen_navi_version, self.constants.whatevergreen_navi_path)

       if self.model == "iMac9,1":
           logging.info("添加 iMac9,1 亮度控制和 DRM 补丁")
       ...
       if not self.constants.custom_model:
           if self.computer.dgpu.device_id == 0x7340:
               logging.info(f"添加 AMD RX5500XT vBIOS 注入")
               ...
               logging.info(f"添加 AMD RX5500XT 启动参数")
               ...
               logging.info(f"添加 AMD WX3200 设备仿冒")
    ...
    def _spoof_handling(self) -> None:
        ...
        if self.model == "MacBookPro9,1":
            logging.info("正在添加 AppleMuxControl 覆盖")
        ...
        if self.model not in model_array.NoAGPMSupport:
            logging.info("正在添加 AppleGraphicsPowerManagement 覆盖")
        ...
        if self.model in model_array.AGDPSupport:
            logging.info("正在添加 AppleGraphicsDevicePolicy 覆盖")
        ...
        if self.model.startswith("iMac14,1"):
            # 确保 agdpmod 应用于仅带有 iGPU 的 iMac14,x
    ...
    def _imac_mxm_patching(self) -> None:
        ...
        if self.constants.metal_build is True:
            logging.info("根据请求添加 Metal GPU 补丁")
            ...
            logging.info(f"检测到 dGPU: {utilities.friendly_hex(self.computer.dgpu.vendor_id)}:{utilities.friendly_hex(self.computer.dgpu.device_id)}")
    ...
    def _ioaccel_workaround(self) -> None:
        ...
        当磁盘上缺少 MTL 包时，WindowServer 将会反复崩溃
        这主要发生在安装 RSR 更新时，其中根目录被清理但 AuxKC 未被清理
        """
        ...
        # 检查是否存在 KDKless 和 KDK GPU
        # 我们仅希望在没有 KDK GPU 的情况下使用 KDKless.kext
        ...
        for arch in gpu_archs:
            ...
            if arch in [
                ...
                if (
                    ...
                    # MacBookPro13,3 具有 AVX2.0 但是 GPU 具有不受支持的帧缓冲区
                    has_kdk_gpu = True

        if has_kdkless_gpu is True and has_kdk_gpu is False:
            # KDKlessWorkaround 对于 KDKless GPU 是必需的
        ...
        # KDKlessWorkaround 支持在 Ventura 上禁用预-AVX2.0 CPU 的原生 AMD 栈
        # 适用于 Polaris, Vega, Navi GPU
