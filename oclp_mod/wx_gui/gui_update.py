...
class UpdateFrame(wx.Frame):
    ...
    def __init__(self, parent: wx.Frame, title: str, global_constants: constants.Constants, screen_location: wx.Point, url: str = "", version_label: str = "") -> None:
        ...
        self.pkg_download_path = self.constants.payload_path / "OCLP-Mod.pkg"
        ...
        def _fetch_update() -> None:
            ...
            file_name = "OCLP-Mod.pkg.zip" if url.endswith(".zip") else "OCLP-Mod.pkg"
            cnurl = url.replace("https://github.com/", "https://download.simplehac.cn/https://github.com/")
            download_obj = network_handler.DownloadObject(cnurl, self.constants.payload_path / file_name)
        ...
        gui_download.DownloadFrame(
            ...
            item_name=f"OCLP-Mod {version_label}",
    ...
    def _extract_update(self) -> None:
        ...
        result = subprocess.run(
            ["/usr/bin/ditto", "-xk", str(self.constants.payload_path / "OCLP-Mod.pkg.zip"), str(self.constants.payload_path)], capture_output=True
        )
